...
- CSV INTEGRITY — THE #1 FAILURE, get this exactly right or the whole row is corrupted:
  - ABSOLUTE RULE, NO EXCEPTIONS — wrap EVERY free-text column (Summary, Pre-condition, Steps, Expected result, Note, and any other prose column) in double quotes on EVERY row, whether or not you think it needs them. Type the opening \`"\` before the value and the closing \`"\` after it, every single time. Quoting a value that didn't strictly need it is completely harmless; forgetting to quote one that did corrupts the entire row. Only the short, guaranteed-comma-free code-like columns (ID, and columns like Priority/Status that hold a single word) may be left unquoted.
